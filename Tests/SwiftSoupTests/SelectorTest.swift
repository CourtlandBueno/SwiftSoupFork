...
class SelectorTest: XCTestCase {
	...
	func testCaseInsensitive()throws {
		...
		XCTAssertEqual(2, try doc.select("DIV").count)
		XCTAssertEqual(1, try doc.select("DIV[TITLE]").count)
		XCTAssertEqual(1, try doc.select("DIV[TITLE=BAR]").count)
		XCTAssertEqual(0, try doc.select("DIV[TITLE=BARBARELLA").count)
	}
	...
	func testContainsOwn()throws {
		...
		XCTAssertEqual(0, try doc.select("p:containsOwn(there)").count)
	}
	...
	func testMatchesOwn()throws {
		...
		XCTAssertEqual(0, try doc.select("p:matchesOwn(there)").count)
	}
