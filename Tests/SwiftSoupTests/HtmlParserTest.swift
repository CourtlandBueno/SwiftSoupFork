...
class HtmlParserTest: XCTestCase {
	...
	func testDropsUnterminatedTag()throws {
		...
		XCTAssertEqual(0, try doc.getElementsByTag("p").count)
	...
	func testCreatesDocumentStructure()throws {
		...
		XCTAssertEqual(2, doc.child(0).children.count) // html node: head and body
		...
		XCTAssertEqual(0, try body.getElementsByTag("meta").count)
	...
	func testDoesNotCreateImplicitLists()throws {
		...
		XCTAssertEqual(0, try doc2.select("ul").count)
		XCTAssertEqual(1, try doc2.select("ol").count)
		XCTAssertEqual(2, try doc2.select("ol li").count)
		XCTAssertEqual(2, try doc2.select("ol li p").count)
		XCTAssertEqual(1, try doc2.select("ol li").get(0).children.count) // one p in first li
	}
	...
	func testHandlesMultiClosingBody()throws {
		...
		let doc: Document = try SwiftSoup.parse(h)
		XCTAssertEqual(3, try doc.select("p").count)
	...
	func testHandlesUnclosedDefinitionLists()throws {
		...
		let doc = try SwiftSoup.parse(h)
		XCTAssertEqual(0, try doc.select("dl").count) // no auto dl
		XCTAssertEqual(4, try doc.select("dt, dd").count)
	...
	func testhandlesSpanInTbody()throws {
		...
		let doc = try SwiftSoup.parse(h)
		XCTAssertEqual(try doc.select("span").first!.children.count, 0) // the span gets closed
		XCTAssertEqual(try doc.select("table").count, 1) // only one table
	}
