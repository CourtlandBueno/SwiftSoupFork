...
class AttributeParseTest: XCTestCase {
	...
	func testhandlesNewLinesAndReturns()throws {
		...
		XCTAssertEqual(2, el.getAttributes()?.count)
	...
	func testdropsSlashFromAttributeName()throws {
		...
		XCTAssertTrue(try doc.select("img[onerror]").count != 0, "SelfClosingStartTag ignores last character")
