...
class NodeTraversor {
     ...
     * - Parameter visitor: a class implementing the {@link NodeVisitor} interface, to be called when visiting each node.
     */
    ...
    open func traverseResult(_ root: Node?) -> Result<Void,Swift.Error> {
        return .init(catching: {try self.traverse(root) })
    }
