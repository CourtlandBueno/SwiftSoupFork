...
public class TreeBuilder {
    ...
    func parseResult(_ input: String, _ baseUri: String, _ errors: ParseErrorList, _ settings: ParseSettings) -> Result<Document,Swift.Error> {
        return .init(catching: { try self.parse(input, baseUri, errors, settings) })
    }
    ...
    public var runParserResult: Result<Void, Swift.Error> {
        return .init(catching: TreeBuilder.runParser(self) )
    }
