...
public class OrderedDictionary<Key: Hashable, Value: Equatable>: MutableCollection, Hashable {
    ...
    internal init(orderedKeys: [Key], keysToValues: [Key: Value]) {
...
class OrderedCodableDictionary<Key: Hashable,Value: Equatable>: OrderedDictionary<Key,Value>, Codable where Key: Codable, Value: Codable {
    ...
    required public init(from decoder: Decoder) throws {
        ...
        let orderedKeys =  try container.decode([Key].self, forKey: .orderedKeys)
        let keysToValues = try container.decode([Key:Value].self, forKey: .keysToValues)
        super.init(orderedKeys: orderedKeys, keysToValues: keysToValues)
    }
    
    public func encode(to encoder: Encoder) throws {
        var container = encoder.container(keyedBy: CodingKeys.self)
        try container.encode(_keysToValues, forKey: .keysToValues)
        try container.encode(_orderedKeys, forKey: .orderedKeys)
    }
    
    override func clone() -> OrderedCodableDictionary<Key, Value> {
        return copy() as! OrderedCodableDictionary<Key,Value>
    }
    
    
}
public struct CodableOrderedDictionaryIterator<Key: Hashable, Value: Equatable>: IteratorProtocol where Key: Codable, Value: Codable {
    
    /// Elements reference
    let orderedDictionary: OrderedCodableDictionary<Key, Value>
    //current element index
    var index = 0
    
    /// Initializer
    init(_ od: OrderedCodableDictionary<Key, Value>) {
        self.orderedDictionary = od
    }
