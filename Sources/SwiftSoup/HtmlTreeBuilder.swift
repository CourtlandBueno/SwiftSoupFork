...
class HtmlTreeBuilder: TreeBuilder {
    ...
    override func parseResult(_ input: String, _ baseUri: String, _ errors: ParseErrorList, _ settings: ParseSettings) -> Result<Document,Swift.Error> {
        let f = HtmlTreeBuilder.parse(self)
        return .init(catching: { try f(input, baseUri, errors, settings) })
    }
    ...
    func parseFragmentResult(_ inputFragment: String, _ context: Element?, _ baseUri: String, _ errors: ParseErrorList, _ settings: ParseSettings) -> Result<Array<Node>,Swift.Error> {
        let f = HtmlTreeBuilder.parseFragment(self)
        return .init(catching: { try f(inputFragment, context, baseUri, errors, settings) })
    }
    ...
    func maybeSetBaseUriResult(_ base: Element) -> Result<Void,Swift.Error> {
        let f = HtmlTreeBuilder.maybeSetBaseUri(self)
        return .init(catching: { try f(base) })
    }
