...
//            try Validate.fail(msg: "Unexpected token type: " + token.tokenType())
    ...
    func parseFragmentResult(_ inputFragment: String, _ baseUri: String, _ errors: ParseErrorList, _ settings: ParseSettings) -> Result<Array<Node>,Swift.Error> {
        return .init(catching: { try self.parseFragment(inputFragment, baseUri, errors, settings)
        })
    }
}
