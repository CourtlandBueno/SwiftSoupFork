...
open class Element: Node {
    ...
    public required init(original: Element, items: [Attribute]) {
        self._tag = original.tag()
        super.init(original.getBaseUri(), Attributes(items), original.childNodes)
    }

    internal init(_ html: String, _ baseUri: String) throws {
        let doc = try parse(html, baseUri)
        self._tag = doc.tag()
        super.init(baseUri, doc.getAttributes() ?? Attributes(), doc.childNodes)
    }
     ...
     * @see #childNodes()
     */
    private func getChildren() -> Elements {
    ...
    public func select(_ cssQuery: String)throws->Elements {
        let selection = try Selector.select(cssQuery, self)
        return selection
    }
    
    public func selectResult(_ query: String) -> Result<Elements,Swift.Error> {
        let f = Element.select(self)
    ...
    public func cssSelector()throws->String {
        ...
        if (parent == nil || ((parent as? Document) != nil)) // don't add Document to selector, as will always have a html node
        {
            ...
            result = try parent!.cssSelector() + (selector)
        }
        return result
    }
    ...
    public func text()throws->String {
        ...
        let result = accum.toString().trim()
        return result
    }
